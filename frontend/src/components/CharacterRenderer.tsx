...
export function CharacterRenderer({ data }: CharacterRendererProps) {
  ...
  console.log("解析后的characterData:", characterData);

  if (!characterData) {
    console.log("CharacterRenderer: characterData为空");
    return null;
  }
  ...
  console.log("最终提取的字段:", { character, basicInfo, lifeExperience });
          ...
          >
            ...
            <ul style={{ margin: 0, paddingLeft: "20px" }}>
              {character.relationships.map((rel: any, index: number) => (
                ...
                >
                  <strong style={{ color: "#f8fafc" }}>{rel.target}</strong> (
