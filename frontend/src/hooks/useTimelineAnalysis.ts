...
export function useTimelineAnalysis() {
  ...
  const checkTimelineData = (data: any): boolean => {
    if (!data) return false;

    if (data.success && data.response) {
      try {
        const parsedResponse = JSON.parse(data.response);
        if (parsedResponse.tool === "extract_timeline") {
          return true;
        }
      } catch (e) {
    ...
    return !!(
      data.data?.key_events ||
      data.data?.events ||
      data.tool === "extract_timeline" ||
      data.data?.tool === "extract_timeline"
    );
  };
